...
import { clearSignoffState } from "./signoffStore.js";

const APPROVED_OR_PARTIAL = /^(Partially Approved|Approved \(Baseline \d+\))$/;

/**
 * Any edit invalidates whatever approval currently applies, regardless of which baseline it
 * names — an edit to a five-cycles-old approval stales it exactly as much as a fresh one.
 * Written untracked, unlike the paragraph edit: the backing signoff record is cleared
 * outright, so rejecting the revision in Word must not silently restore a status that no
 * longer has an approval behind it.
 */
...
export async function applyChange(
  ...
  const { author, docId, draftName } = options;
  const { zip, doc, docXmlPath } = await loadDocxDocument(filePath);
